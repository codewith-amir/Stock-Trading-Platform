 ...
 * ╠═══════════════════════════════════════════════════════════════════╣
 * ║  Features:                                                       ║
 * ║   • 12 real-world stocks (Tech, Finance, EV, Healthcare, etc.)  ║
 * ║   • Live market simulation  (±3% price fluctuation per tick)    ║
 * ║   • Buy / Sell shares with balance enforcement                  ║
 * ║   • Portfolio P&L tracking (realized + unrealized)              ║
 * ║   • Day High/Low, Volume, Price History per stock               ║
 * ║   • Transaction history with timestamps                         ║
 * ║   • File I/O  — saves CSV snapshots of trades & portfolio       ║
 * ║   • Full OOP design  — 7 classes across 3 packages              ║
 * ╠═══════════════════════════════════════════════════════════════════╣
...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        // Initial market refresh so prices move from seed values
        ...
        while (running) {
            ...
            switch (choice) {
                ...
                case "0" -> {
                    System.out.println();
                    DisplayHelper.info("Auto-saving data before exit...");
                    FileService.saveTransactionHistory(user);
                    FileService.savePortfolioSnapshot(user, market);
                    System.out.println();
                    DisplayHelper.info("Goodbye, " + user.getUsername() + "! Happy trading!");
                    System.out.println();
                    running = false;
                }
    ...
    private static void buyShares() {
        ...
        try {
            ...
            if (input.isEmpty()) { DisplayHelper.err("Quantity cannot be empty."); return; }
            qty = Integer.parseInt(input);
            if (qty <= 0) { DisplayHelper.err("Quantity must be positive."); return; }
            if (qty > 1_000_000) { DisplayHelper.err("Quantity too large (max 1,000,000)."); return; }
            if (qty > maxShares) { DisplayHelper.err("Insufficient funds. Maximum: " + maxShares + " shares."); return; }
        } catch (NumberFormatException e) {
            DisplayHelper.err("Invalid quantity. Please enter a valid integer."); return;
        }

        // Confirm
        double total = qty * stock.getCurrentPrice();
        System.out.printf("  Confirm: BUY %d × %s @ $%.2f  =  " +
            DisplayHelper.BOLD + "$%.2f" + DisplayHelper.RESET + "  (y/n): ",
            qty, symbol, stock.getCurrentPrice(), total);
        String confirm = scanner.nextLine().trim().toLowerCase();

        if (!confirm.equals("y")) { DisplayHelper.info("Order cancelled."); return; }
    ...
    private static void sellShares() {
        ...
        Stock stock = market.getStock(symbol);
        if (stock == null) {
            DisplayHelper.err("Symbol '" + symbol + "' not found."); return;
        }

        PortfolioHolding holding = user.getHolding(symbol);
        if (holding == null) {
            DisplayHelper.err("You don't own any shares of " + symbol + "."); return;
        }
        ...
        try {
            ...
            qty = Integer.parseInt(input);
            if (qty <= 0) { DisplayHelper.err("Quantity must be positive."); return; }
            if (qty > ownedShares) { DisplayHelper.err("Cannot sell " + qty + " shares. You only own " + ownedShares + "."); return; }
        } catch (NumberFormatException e) {
            DisplayHelper.err("Invalid quantity. Please enter a valid integer."); return;
        }
        ...
        System.out.printf("  Confirm: SELL %d × %s @ $%.2f  =  " +
